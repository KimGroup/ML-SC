...
$j_x({\bf r_1})= -i[H({\bf r}_1),\hat{x}]$. The Fourier transform of the 
current-current correlation function in Eq.~\eqref{eq:Lambda} is famously\cite{Scalapino1993} related to the superfluid density $\rho_s$ through $\rho_s\propto \Lambda_{xx}(q_x\rightarrow 0,q_y=0,\omega_n=0)-\Lambda_{xx}(q_x=0,q_y\rightarrow0,\omega_n=0)$. 
...
%\label{eq:lambdaform}
%\end{eqnarray}
Now a rigorous relationship between the Loop operators $L^\Box_{jklm}$ and $ L^\triangle_{jkl}$  and the longitudinal current response can be established for a mean-field Bogoliubov deGenne Hamiltonian $H'=-\hat{P}$ with flat band, where
%For a gapped Hamiltonian, the Hamiltonian can be approximated by a flat band Hamiltonian  $H'=-\hat{P}$  with the 
%if we consider
%the projection operator
$\hat{P}\equiv |G\rangle \langle G|$ is the projection operator onto the ground state $|G\rangle$. Now for the system with the Hamiltonian $H'$ at zero temperature %, the right hand side of Eq.~\ref{eq:lambdaform} simplifies to 
...
&=&  \sum_{r_3 r_4}P_{r_2 r_4}P_{r_4r_1}P_{r_1r_3}P_{r_3r_2}\left(x_1-x_4\right)\left(x_2-x_3\right)\nonumber \\
& & - \sum_{r_4}P_{r_2 r_4}P_{r_4r_1}P_{r_1r_2}\left(x_1-x_4\right)\left(x_2-x_1\right)
\label{eq:j-j}
\end{eqnarray}
where $P_{r'r}= \langle G|c^\dagger_{r'} c_r|G\rangle $. See Appendix for further details.
...
To test the potential of the QLT for efficiently detecting qualitative difference in the transport from imaginary time Greens function data, we consider models that host superconductivity in parts of the phase space. {\color{blue}Due to the Kosterlitz–Thouless type transition and the absence of accessible superconducting order parameter in two dimensions, a rigorous characterization within DQMC commonly relies on the calculation of the superfluid density. While a non-zero superfluid density signals the onset of diamagnetism thus the existence of superconducting fluctuation, the actual onset of the superconducting order requires the superfluid density to surpass certain value proportional to the transition temperature.}
...
In this appendix, we explore the connection between the zero-frequency current-current correlation in Eq. \ref{eq:Lambda} and the products of two-point correlators trailing loops that are present in quantum loop topography:
\begin{eqnarray}
...
\label{eq:app1}
\end{eqnarray}
For simplicity, starting on the third line we have been focusing on a flat-band mean-field Hamiltonian $H'=-\hat P$, $\hat P=\sum_{ij}P_{ij}c^\dagger_i c_j=\sum_{\mu}c^\dagger_\mu c_\mu$. $\left|\mu\right\rangle$ ($\left|\nu\right\rangle$) are the single-particle states that are occupied (empty) in the ground state $\left|G\right\rangle$ of $H'$. It follows that $\epsilon_\mu=-1$, $\epsilon_\nu=0$, and $1-\hat P=\sum_{\nu}c^\dagger_\nu c_\nu=\sum_{\nu}\left|\nu\right\rangle\left\langle\nu\right|$. The $\hat{x}$ direction current operator at position $r$ is $j_x(r)=-i\left[H'(r),\hat x\right]=\sum_{r'}iP_{r'r}c_{r'}^\dagger c_{r}\left(x-x'\right)+\mbox{h.c.}$. Putting it into Eq.\ref{eq:app1}, we have :
\begin{eqnarray}
& &\mbox{tr}\left[\hat{P} j_x(r_1) \left(1-\hat{P}\right) j_x(r_2)\right] \nonumber \\
&=&  \sum_{r_3 r_4 r'_1 r'_2r'_3r'_4}P_{r'_4 r_4}P_{r'_1r_1}\left(P_{r'_3r_3}-\delta_{r'_3r_3}\right)P_{r'_2r_2}\left(x_1-x'_1\right)\left(x_2-x'_2\right)\nonumber \\& &\times \left\langle0\left|c_{r_4}c^\dagger_{r'_1} c_{r_1} c^\dagger_{r'_3} c_{r_3} c^\dagger_{r'_2} c_{r_2} c^\dagger_{r'_4}\right|0\right\rangle\nonumber \\
&=&  \sum_{r_3 r_4}P_{r_2 r_4}P_{r_4r_1}P_{r_1r_3}P_{r_3r_2}\left(x_1-x_4\right)\left(x_2-x_3\right)\nonumber \\
& & - \sum_{r_4}P_{r_2 r_4}P_{r_4r_1}P_{r_1r_2}\left(x_1-x_4\right)\left(x_2-x_1\right)
\label{eq:app2}
\end{eqnarray}
...
\begin{eqnarray}
\left\langle c_{r'}^{\dagger}c_{r}\right\rangle & = & \sum_{\mu}\left\langle \mu|c_{r'}^{\dagger}c_{r}|\mu\right\rangle \nonumber\\
 & = & \sum_{\tilde{r}' \tilde{r}}P_{\tilde{r}\tilde{r}'}\left\langle 0|c_{\tilde{r}'}c_{r'}^{\dagger}c_{r}c_{\tilde{r}}^{\dagger}|0\right\rangle  =P_{rr'}
\end{eqnarray}
Therefore, Eq. \ref{eq:app2} can be expressed in terms of products of two-point correlators trailing triangles and quadrilaterals. 
